...
function loadDoctors() {
    fetch(API_BASE + 'get_doctors.php')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                const select = document.getElementById('appointment_doctor');
...
document.getElementById('appointmentForm').addEventListener('submit', function(e) {
    ...
    const data = {
        doctor_id: document.getElementById('appointment_doctor').value,
        appointment_date: document.getElementById('appointment_date').value,
        reason: document.getElementById('appointment_reason').value
    };
    
    if (!data.doctor_id || !data.appointment_date || !data.reason) {
    ...
    .then(data => {
        if (data.success) {
            showAlert('Đăng ký lịch khám thành công', 'success');
    ...
    fetch(API_BASE + 'get_appointments.php')
        ...
        .then(data => {
            if (data.success) {
                ...
                let html = '<div class="table-container"><table><thead><tr><th>Bác sĩ</th><th>Chuyên khoa</th><th>Ngày giờ</th><th>Lý do</th><th>Trạng thái</th></tr></thead><tbody>';
